...
object HotzoneAnalysis {
  ...
  def runHotZoneAnalysis(spark: SparkSession, pointPath: String, rectanglePath: String): DataFrame = {
    ...
    val hotnessDf = spark.sql("select rectangle, count(point) as hotness from joinResult group by rectangle order by rectangle asc")
    hotnessDf.createOrReplaceTempView("sortedResult")


    return hotnessDf // YOU NEED TO CHANGE THIS PART
  }
